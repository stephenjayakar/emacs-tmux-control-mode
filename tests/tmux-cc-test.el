...
(ert-deftest tmux-cc-setup-keybindings-test ()
  (let ((tmux-cc-focus-next-key "C-<tab>")
    ...
    (should (eq (lookup-key tmux-cc-pane-mode-map (kbd "C-x o"))
                #'tmux-cc-focus-next-pane))
    (should (eq (lookup-key tmux-cc-pane-mode-map (kbd "C-c |"))
                #'tmux-cc-split-horizontal))
    (should (eq (lookup-key tmux-cc-pane-mode-map (kbd "C-c -"))
                #'tmux-cc-split-vertical))
    (should (eq (lookup-key tmux-cc-pane-mode-map (kbd "C-c C-n"))
                #'tmux-cc-new-window))
    (should (eq (lookup-key tmux-cc-pane-mode-map (kbd "C-c N"))
                #'tmux-cc-new-session))
    (should (eq (lookup-key tmux-cc-pane-mode-map (kbd "C-c w"))
                #'tmux-cc-manager))))
